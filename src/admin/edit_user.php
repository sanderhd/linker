...
// als er geen user is terug naar index
if (!$user) {
	header("Location: users.php");
	exit();
}
			...
			<div class="bg-neutral-800 shadow-lg rounded-2xl border border-white/10 p-8 max-w-md mx-auto">
				<div class="text-center mb-6">
					<div class="flex items-center justify-center gap-3 mb-2">
						...
						<h1 class="text-3xl font-bold text-white">Edit <span class="text-orange-500">User</span></h1>
					</div>
				</div>
				...
				<form method="POST" class="space-y-4">
					<div>
						<label for="username" class="block text-white/90 font-medium mb-2">Username:</label>
						<input type="text" id="username" name="username" value="<?php echo htmlspecialchars($user['username'] ?? ''); ?>" placeholder="Type your username..." class="w-full px-4 py-2 bg-neutral-700 border border-white/10 rounded text-white placeholder-white/40 focus:outline-none focus:border-orange-500" required />
					</div>

					<div>
						<label for="email" class="block text-white/90 font-medium mb-2">Email:</label>
						<input type="email" id="email" name="email" value="<?php echo htmlspecialchars($user['email'] ?? ''); ?>" placeholder="Type your e-mail..." class="w-full px-4 py-2 bg-neutral-700 border border-white/10 rounded text-white placeholder-white/40 focus:outline-none focus:border-orange-500" required />
					</div>
